...
export class RegisterStep2Component implements OnInit {
  ...
  private geofenceCircle: L.Circle | undefined;
  private geofenceCenter = { lat: 36.8460657, lng: 10.2678393 };
  private geofenceRadius = 700; 

  private geofencePolygon: L.Polygon | undefined;
  ...
  private initMap(): void {
    this.map = L.map('map').setView([36.8, 10.3], 11);
    ...
    this.geofencePolygon = L.polygon(this.geofencePolygonCoords, {
      color: 'blue',
      weight: 3,
      dashArray: '8, 8',
      fillOpacity: 0.05,
  ...
  reverseGeocode(lat: number, lng: number) {
    ...
    if (!this.isPointInPolygon(lat, lng, this.geofencePolygonCoords)) {
      alert('Selected location is outside Les Berges Du Lac II geofence!');
